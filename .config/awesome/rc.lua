...
-- Table of layouts to cover with awful.layout.inc, order matters./etc/xdg/awesome/rc.lua
...
battery2 = battery_widget({adapter = "BAT1", timeout=10, listen=true, ac_prefix=" ", battery_prefix=" "})
...
globalkeys = gears.table.join(
    ...
    awful.key({ alt },               "F2",    function () awful.screen.focused().mypromptbox:run() end,
              {description = "run prompt", group = "launcher"}),

    -- Screen lock (this is properly handled by xss-lock started below)
    ...
    awful.key({}, "XF86Tools", function() awful.util.spawn("pactl set-source-mute alsa_input.pci-0000_00_1f.3.analog-stereo toggle") end,
              {description = "mute microphone", group = "custom"}),
...
awful.rules.rules = {
    ...
    { rule = { class = "volumeicon"},
      properties = { placement = awful.placement.right + awful.placement.bottom, floating = true }},
}
-- }}}
